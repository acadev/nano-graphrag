"""
Local, global, and naive query pipelines.

Extracted from _op.py; see _extraction.py and _reporting.py for the other
two concerns.
"""
...
async def _find_most_related_text_unit_from_entities(
    ...
    all_one_hop_nodes: set = set()
    for this_edges in edges:
        if not this_edges:
            continue
        all_one_hop_nodes.update([e[1] for e in this_edges])
    all_one_hop_nodes_list = list(all_one_hop_nodes)
    all_one_hop_nodes_data = await knowledge_graph_inst.get_nodes_batch(
        all_one_hop_nodes_list
    )
    all_one_hop_text_units_lookup = {
        k: set(split_string_by_multi_markers(v["source_id"], [GRAPH_FIELD_SEP]))
        for k, v in zip(all_one_hop_nodes_list, all_one_hop_nodes_data)
        if v is not None
    }
    all_text_units_lookup: dict = {}
...
async def _build_local_query_context(
    ...
    logger.info(
        f"Using {len(node_datas)} entities, {len(use_communities)} communities, "
    ...
    for i, n in enumerate(node_datas):
        entities_section_list.append(
            [i, n["entity_name"], n.get("entity_type", "UNKNOWN"), n.get("description", "UNKNOWN"), n["rank"]]
        )
...
async def local_query(
    ...
    return await use_model_func(query, system_prompt=sys_prompt)
...
async def _map_global_communities(
    ...
    async def _process(community_truncated_datas: list[CommunitySchema]) -> list:
        communities_section_list = [["id", "content", "rating", "importance"]]
        for i, c in enumerate(community_truncated_datas):
            communities_section_list.append(
                [i, c["report_string"], c["report_json"].get("rating", 0), c["occurrence"]]
            )
...
async def global_query(
    ...
    sorted_community_schemas = sorted(
        community_schema.items(), key=lambda x: x[1]["occurrence"], reverse=True
    ...
    final_support_points = [
        {"analyst": i, "answer": point["description"], "score": point.get("score", 1)}
        for i, mc in enumerate(map_communities_points)
        for point in mc
        if "description" in point
    ]
    final_support_points = [p for p in final_support_points if p["score"] > 0]
    if not final_support_points:
        return PROMPTS["fail_response"]
