"""
Community-report generation pipeline.

Extracted from _op.py; see _extraction.py and _querying.py for the other
two concerns.
"""
...
# ---------------------------------------------------------------------------
# Fallback report (used when LLM community-report call fails)
# ---------------------------------------------------------------------------
...
def _fallback_community_report(community: SingleCommunitySchema, describe: str) -> dict:
    ...
    return {
        ...
        "summary": (
            f"Fallback report for a community with {len(community['nodes'])} nodes and "
            f"{len(community['edges'])} relationships. Top entities: {top_nodes}."
        ),
        "findings": [
            {"summary": "Top entities", "explanation": top_nodes},
            {"summary": "Evidence summary", "explanation": describe[:1200]},
        ],
        "rating": 1,
    }
...
def _pack_single_community_by_sub_communities(
    ...
    sub_communities_describe = list_of_list_to_csv(
        [sub_fields]
        + [
            [i, c["report_string"], c["report_json"].get("rating", -1), c["occurrence"]]
...
```"""
    ...
    report_describe = ""
    contain_nodes: set = set()
    contain_edges: set = set()

    truncated = len(nodes_in_order) > 100 or len(edges_in_order) > 100
    need_to_use_sub_communities = (
        truncated and community["sub_communities"] and already_reports
    )
    ...
    nodes_list_data = [
        [i, name, data.get("entity_type", "UNKNOWN"), data.get("description", "UNKNOWN"), node_degrees[i]]
...
def _community_report_json_to_str(parsed_output: dict) -> str:
    """Convert a structured community-report JSON to a markdown string.

    Mirrors the official graphrag: index/graph/extractors/community_reports.
    """
    title = parsed_output.get("title", "Report")
    summary = parsed_output.get("summary", "")
    findings = parsed_output.get("findings", [])

    def finding_summary(f: dict | str) -> str:
        return f if isinstance(f, str) else f.get("summary", "")

    def finding_explanation(f: dict | str) -> str:
        return "" if isinstance(f, str) else f.get("explanation", "")
...
# ---------------------------------------------------------------------------
# Main community-report pipeline
# ---------------------------------------------------------------------------
...
async def generate_community_report(
    ...
    for level in levels:
        ...
        this_level_keys, this_level_values = zip(*this_level_pairs)
        this_level_reports = await asyncio.gather(
            *[_form_single_community_report(c, community_datas) for c in this_level_values]
        )
        community_datas.update(
            {
                ...
                for k, r, v in zip(this_level_keys, this_level_reports, this_level_values)
            }
        )
